...
class SpiderPipeline:
    ...
    def convert_to_filename_format(self, datetime_list):
        ...
        for datetime_str in datetime_list:
            # 替换 / 和 : 为 -
            filename_str = (
                datetime_str.replace("/", "").replace(":", "").replace("-", "")
            )
            # 去除空格
...
class HistoryPipeline:
    ...
    def open_spider(self, spider):
        print("HistoryPipeline open_spider")
        self.items = []
...
class RolePipeline:
    ...
    def open_spider(self, spider):
        print("RolePipeline open_spider")
        self.items = []

    def process_item(self, item, spider):
        # print(f"==================> {item}")
        # print(f"下载图片")
        # file_name = f"{img_dir}/{spider.name}/{item['title']}/simple.png"
